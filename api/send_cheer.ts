...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  db().prepare(
    "INSERT INTO cheers (room_id, type, from_nick) VALUES (?, ?, ?)"
  ).run(roomId, type, fromNick);

  db().prepare(
    "DELETE FROM cheers WHERE room_id = ? AND created_at < datetime('now', '-1 minute')"
  ).run(roomId);

  jsonSuccess(res, { message: 'Cheer sent' });
}
